...
Hacky workaround for the error below:
  ...
  /home/git_repos/glen-amd/flashinfer/include/flashinfer/attention/../vec_dtypes_hip.cuh:200:38: error: use of
undeclared identifier '__float2bfloat162_rn'; did you mean '__float22bfloat162_rn'? 200 |       const gpu_bfloat162
bias_reg = __float2bfloat162_rn(*reinterpret_cast<const float*>(&BIAS)); | ^~~~~~~~~~~~~~~~~~~~ | __float22bfloat162_rn
  /opt/rocm-6.3.1/lib/llvm/bin/../../../include/hip/amd_detail/amd_hip_bf16.h:574:45: note: '__float22bfloat162_rn'
declared here 574 | __BF16_HOST_DEVICE_STATIC__ __hip_bfloat162 __float22bfloat162_rn(const float2 a) {
*/
...
SGL_HIP_INLINE __gpu_bfloat162 make_bfloat162(const __gpu_bfloat16 x, const __gpu_bfloat16 y) {
  __gpu_bfloat162 t;
  t.x = x;
  t.y = y;
  return t;
...
#define SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED
#endif
...
SGL_HIP_INLINE __gpu_bfloat16 __hmul(const __gpu_bfloat16 a, const __gpu_bfloat16 b) {
  __gpu_bfloat16 val;
...
SGL_HIP_INLINE __gpu_bfloat162 __hmul2(const __gpu_bfloat162 a, const __gpu_bfloat162 b) {
  __gpu_bfloat162 val;
...
SGL_HIP_INLINE __gpu_bfloat162 __floats2bfloat162_rn(const float a, const float b) {
  __gpu_bfloat162 val;
  val = __gpu_bfloat162(__float2bfloat16_rn(a), __float2bfloat16_rn(b));
  return val;
}

SGL_HIP_INLINE __gpu_bfloat162 __float22bfloat162_rn(const float2 a) {
  __gpu_bfloat162 val = __floats2bfloat162_rn(a.x, a.y);
  return val;
}
SGL_HIP_INLINE float2 __bfloat1622float2(const __gpu_bfloat162 a) {
  float hi_float;
  float lo_float;
...
/*******************************************************/
// to_float: generic -> float
template <typename U>
SGL_HIP_INLINE float to_float(U v) {
  return static_cast<float>(v);
}

// half specialization
template <>
SGL_HIP_INLINE float to_float<half>(half v) {
  return __half2float(v);
}

// from_float: float -> generic
template <typename U>
SGL_HIP_INLINE U from_float(float v) {
  return static_cast<U>(v);
}

// half specialization
template <>
SGL_HIP_INLINE half from_float<half>(float v) {
...
struct vec_cast {
  ...
  SGL_HIP_INLINE static void cast(dst_t* dst, const src_t* src) {
#pragma unroll
...
struct vec_cast<float, half> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(float* dst, const half* src) {
...
struct vec_cast<half, float> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(half* dst, const float* src) {
    if constexpr (vec_size == 1) {
...
template <typename T>
constexpr SGL_HIP_INLINE int get_exponent_bits() {
...
template <typename T>
constexpr SGL_HIP_INLINE int get_mantissa_bits() {
  if constexpr (std::is_same_v<T, __gpu_fp8_e4m3>) {
    return 3;
  } else if constexpr (std::is_same_v<T, __gpu_fp8_e5m2>) {
    return 2;
...
__device__ void fast_dequant_f8f16x4(uint32_t* input, uint2* output) {
  ...
  if constexpr (std::is_same_v<fp8_dtype, __gpu_fp8_e5m2> && std::is_same_v<fp16_dtype, half>) {
    ...
    if constexpr (std::is_same_v<fp16_dtype, half>) {
      ...
      *(gpu_bfloat162*)&(output->x) = __hmul2(*reinterpret_cast<const gpu_bfloat162*>(&Out1), bias_reg);
      *(gpu_bfloat162*)&(output->y) = __hmul2(*reinterpret_cast<const gpu_bfloat162*>(&Out2), bias_reg);
    }
  }
}
...
struct vec_cast<gpu_bfloat16, __gpu_fp8_e4m3> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(gpu_bfloat16* dst, const __gpu_fp8_e4m3* src) {
    if constexpr (vec_size == 1) {
...
#pragma unroll
      for (uint32_t i = 0; i < vec_size / 4; ++i) {
        fast_dequant_f8f16x4<__gpu_fp8_e4m3, gpu_bfloat16>((uint32_t*)&src[i * 4], (uint2*)&dst[i * 4]);
      }
    }
  }
};
...
struct vec_cast<gpu_bfloat16, __gpu_fp8_e5m2> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(gpu_bfloat16* dst, const __gpu_fp8_e5m2* src) {
    if constexpr (vec_size == 1) {
      dst[0] = gpu_bfloat16(src[0]);
    } else if constexpr (vec_size == 2) {
      dst[0] = gpu_bfloat16(src[0]);
...
#pragma unroll
      for (uint32_t i = 0; i < vec_size / 4; ++i) {
        fast_dequant_f8f16x4<__gpu_fp8_e5m2, gpu_bfloat16>((uint32_t*)&src[i * 4], (uint2*)&dst[i * 4]);
      }
    }
  }
};
...
struct vec_cast<__gpu_fp8_e4m3, half> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(__gpu_fp8_e4m3* dst, const half* src) {
#ifdef SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED
    if constexpr (vec_size == 1) {
...
#endif  // SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED
  }
};
...
struct vec_cast<__gpu_fp8_e5m2, half> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(__gpu_fp8_e5m2* dst, const half* src) {
#ifdef SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED
    if constexpr (vec_size == 1) {
...
struct vec_cast<half, __gpu_fp8_e4m3> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(half* dst, const __gpu_fp8_e4m3* src) {
#ifdef SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED  // FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
    if constexpr (vec_size == 1) {
      dst[0] = half(src[0]);
    } else {
#pragma unroll
...
struct vec_cast<half, __gpu_fp8_e5m2> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(half* dst, const __gpu_fp8_e5m2* src) {
#ifdef SGL_HIP_HARDWARE_FP8_CONVERSION_ENABLED
    if constexpr (vec_size == 1) {
      dst[0] = half(src[0]);
    } else {
...
struct vec_cast<float, gpu_bfloat16> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(float* dst, const gpu_bfloat16* src) {
    if constexpr (vec_size == 1) {
      dst[0] = (float)src[0];
    } else {
#pragma unroll
...
struct vec_cast<gpu_bfloat16, float> {
  template <size_t vec_size>
  SGL_HIP_INLINE static void cast(gpu_bfloat16* dst, const float* src) {
    /*if constexpr (vec_size == 1) {
      dst[0] = gpu_bfloat16(src[0]);
    } else {
#pragma unroll
...
struct vec_t {
  ...
  SGL_HIP_INLINE static void memcpy(float_t* dst, const float_t* src);
  SGL_HIP_INLINE float_t* ptr();
};

template <typename src_float_t, typename tgt_float_t, size_t vec_size>
SGL_HIP_INLINE void cast_from_impl(vec_t<tgt_float_t, vec_size>& dst, const vec_t<src_float_t, vec_size>& src) {
#if defined(__HIPCC__) || (defined(__clang__) && defined(__HIP__)) || defined(__HIPCC_RTC__)
...
template <typename src_float_t, typename tgt_float_t, size_t vec_size>
SGL_HIP_INLINE void cast_load_impl(vec_t<tgt_float_t, vec_size>& dst, const src_float_t* src_ptr) {
...
template <typename src_float_t, typename tgt_float_t, size_t vec_size>
SGL_HIP_INLINE void cast_store_impl(tgt_float_t* dst_ptr, const vec_t<src_float_t, vec_size>& src) {
  if constexpr (std::is_same_v<src_float_t, tgt_float_t>) {
    src.store(dst_ptr);
  } else {
...
struct vec_t<__gpu_fp8_e4m3, 1> {
  ...
  SGL_HIP_INLINE __gpu_fp8_e4m3& operator[](size_t i) {
    return ((__gpu_fp8_e4m3*)(&data))[i];
  }
  SGL_HIP_INLINE const __gpu_fp8_e4m3& operator[](size_t i) const {
    return ((const __gpu_fp8_e4m3*)(&data))[i];
  }
  SGL_HIP_INLINE __gpu_fp8_e4m3* ptr() {
    return reinterpret_cast<__gpu_fp8_e4m3*>(&data);
  }
  SGL_HIP_INLINE void fill(__gpu_fp8_e4m3 val);
  SGL_HIP_INLINE void load(const __gpu_fp8_e4m3* ptr);
  SGL_HIP_INLINE void store(__gpu_fp8_e4m3* ptr) const;
  template <typename T>
  ...
  SGL_HIP_INLINE static void memcpy(__gpu_fp8_e4m3* dst, const __gpu_fp8_e4m3* src);
};

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 1>::fill(__gpu_fp8_e4m3 val) {
  data = val;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 1>::load(const __gpu_fp8_e4m3* ptr) {
  data = *ptr;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 1>::store(__gpu_fp8_e4m3* ptr) const {
  *ptr = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 1>::memcpy(__gpu_fp8_e4m3* dst, const __gpu_fp8_e4m3* src) {
  *dst = *src;
}
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 2>::fill(__gpu_fp8_e4m3 val) {
  data.__x = (__gpu_fp8x2_storage_t(val.__x) << 8) | __gpu_fp8x2_storage_t(val.__x);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 2>::load(const __gpu_fp8_e4m3* ptr) {
  data = *((__gpu_fp8x2_e4m3*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 2>::store(__gpu_fp8_e4m3* ptr) const {
  *((__gpu_fp8x2_e4m3*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 2>::memcpy(__gpu_fp8_e4m3* dst, const __gpu_fp8_e4m3* src) {
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 4>::fill(__gpu_fp8_e4m3 val) {
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 4>::load(const __gpu_fp8_e4m3* ptr) {
  data = *((__gpu_fp8x4_e4m3*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 4>::store(__gpu_fp8_e4m3* ptr) const {
  *((__gpu_fp8x4_e4m3*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 4>::memcpy(__gpu_fp8_e4m3* dst, const __gpu_fp8_e4m3* src) {
...
struct vec_t<__gpu_fp8_e4m3, 8> {
  ...
  SGL_HIP_INLINE void store(__gpu_fp8_e4m3* ptr) const;
  template <typename T>
  SGL_HIP_INLINE void cast_from(const vec_t<T, 8>& src) {
    cast_from_impl(*this, src);
  }
  template <typename T>
  SGL_HIP_INLINE void cast_load(const T* ptr) {
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 8>::fill(__gpu_fp8_e4m3 val) {
  ((__gpu_fp8x4_e4m3*)(&data.x))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                        (__gpu_fp8x4_storage_t(val.__x) << 16) | (__gpu_fp8x4_storage_t(val.__x) << 8) |
                                        __gpu_fp8x4_storage_t(val.__x);
  ((__gpu_fp8x4_e4m3*)(&data.y))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                        (__gpu_fp8x4_storage_t(val.__x) << 16) | (__gpu_fp8x4_storage_t(val.__x) << 8) |
                                        __gpu_fp8x4_storage_t(val.__x);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 8>::load(const __gpu_fp8_e4m3* ptr) {
  data = *((uint2*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 8>::store(__gpu_fp8_e4m3* ptr) const {
  *((uint2*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e4m3, 8>::memcpy(__gpu_fp8_e4m3* dst, const __gpu_fp8_e4m3* src) {
  *((uint2*)dst) = *((uint2*)src);
}
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 16; ++i) {
      ((__gpu_fp8x4_e4m3*)(&(data[i].x)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e4m3*)(&(data[i].y)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e4m3*)(&(data[i].z)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e4m3*)(&(data[i].w)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
    }
  }
...
struct vec_t<__gpu_fp8_e5m2, 1> {
  ...
  SGL_HIP_INLINE __gpu_fp8_e5m2& operator[](size_t i) {
    return ((__gpu_fp8_e5m2*)(&data))[i];
  }
  SGL_HIP_INLINE const __gpu_fp8_e5m2& operator[](size_t i) const {
    return ((const __gpu_fp8_e5m2*)(&data))[i];
  }
  SGL_HIP_INLINE __gpu_fp8_e5m2* ptr() {
    return reinterpret_cast<__gpu_fp8_e5m2*>(&data);
  }
  SGL_HIP_INLINE void fill(__gpu_fp8_e5m2 val);
  SGL_HIP_INLINE void load(const __gpu_fp8_e5m2* ptr);
  SGL_HIP_INLINE void store(__gpu_fp8_e5m2* ptr) const;
  template <typename T>
  SGL_HIP_INLINE void cast_from(const vec_t<T, 1>& src) {
    cast_from_impl(*this, src);
  }
  ...
  SGL_HIP_INLINE static void memcpy(__gpu_fp8_e5m2* dst, const __gpu_fp8_e5m2* src);
};

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 1>::fill(__gpu_fp8_e5m2 val) {
  data = val;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 1>::load(const __gpu_fp8_e5m2* ptr) {
  data = *ptr;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 1>::store(__gpu_fp8_e5m2* ptr) const {
  *ptr = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 1>::memcpy(__gpu_fp8_e5m2* dst, const __gpu_fp8_e5m2* src) {
  *dst = *src;
}
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 2>::fill(__gpu_fp8_e5m2 val) {
  data.__x = (__gpu_fp8x2_storage_t(val.__x) << 8) | __gpu_fp8x2_storage_t(val.__x);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 2>::load(const __gpu_fp8_e5m2* ptr) {
  data = *((__gpu_fp8x2_e5m2*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 2>::store(__gpu_fp8_e5m2* ptr) const {
  *((__gpu_fp8x2_e5m2*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 2>::memcpy(__gpu_fp8_e5m2* dst, const __gpu_fp8_e5m2* src) {
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 4>::fill(__gpu_fp8_e5m2 val) {
  data.__x = (__gpu_fp8x4_storage_t(val.__x) << 24) | (__gpu_fp8x4_storage_t(val.__x) << 16) |
             (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 4>::load(const __gpu_fp8_e5m2* ptr) {
  data = *((__gpu_fp8x4_e5m2*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 4>::store(__gpu_fp8_e5m2* ptr) const {
  *((__gpu_fp8x4_e5m2*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 4>::memcpy(__gpu_fp8_e5m2* dst, const __gpu_fp8_e5m2* src) {
...
SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 8>::fill(__gpu_fp8_e5m2 val) {
  ((__gpu_fp8x4_e5m2*)(&data.x))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                        (__gpu_fp8x4_storage_t(val.__x) << 16) | (__gpu_fp8x4_storage_t(val.__x) << 8) |
                                        __gpu_fp8x4_storage_t(val.__x);
  ((__gpu_fp8x4_e5m2*)(&data.y))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                        (__gpu_fp8x4_storage_t(val.__x) << 16) | (__gpu_fp8x4_storage_t(val.__x) << 8) |
                                        __gpu_fp8x4_storage_t(val.__x);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 8>::load(const __gpu_fp8_e5m2* ptr) {
  data = *((uint2*)ptr);
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 8>::store(__gpu_fp8_e5m2* ptr) const {
  *((uint2*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<__gpu_fp8_e5m2, 8>::memcpy(__gpu_fp8_e5m2* dst, const __gpu_fp8_e5m2* src) {
  *((uint2*)dst) = *((uint2*)src);
}
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 16; ++i) {
      ((__gpu_fp8x4_e5m2*)(&(data[i].x)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e5m2*)(&(data[i].y)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e5m2*)(&(data[i].z)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
      ((__gpu_fp8x4_e5m2*)(&(data[i].w)))->__x = (__gpu_fp8x4_storage_t(val.__x) << 24) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 16) |
                                                 (__gpu_fp8x4_storage_t(val.__x) << 8) | __gpu_fp8x4_storage_t(val.__x);
    }
  }
...
struct vec_t<half, 1> {
  ...
  SGL_HIP_INLINE void fill(float v) {
    fill(__float2half(v));
  }  // TODO (Hubert): __float2half
  ...
  SGL_HIP_INLINE static void memcpy(half* dst, const half* src);
};
...
SGL_HIP_INLINE void vec_t<half, 1>::memcpy(half* dst, const half* src) {
  *dst = *src;
}
...
SGL_HIP_INLINE void vec_t<half, 2>::memcpy(half* dst, const half* src) {
  *((half2*)dst) = *((half2*)src);
}
...
SGL_HIP_INLINE void vec_t<half, 4>::memcpy(half* dst, const half* src) {
  *((uint2*)dst) = *((uint2*)src);
}
...
struct vec_t<gpu_bfloat16, 1> {
  gpu_bfloat16 data;
  SGL_HIP_INLINE gpu_bfloat16& operator[](size_t i) {
    return ((gpu_bfloat16*)(&data))[i];
  }
  SGL_HIP_INLINE const gpu_bfloat16& operator[](size_t i) const {
    return ((const gpu_bfloat16*)(&data))[i];
  }
  SGL_HIP_INLINE gpu_bfloat16* ptr() {
    return reinterpret_cast<gpu_bfloat16*>(&data);
  }
  SGL_HIP_INLINE void fill(gpu_bfloat16 val);
  SGL_HIP_INLINE void load(const gpu_bfloat16* ptr);
  SGL_HIP_INLINE void store(gpu_bfloat16* ptr) const;
  template <typename T>
  SGL_HIP_INLINE void cast_from(const vec_t<T, 1>& src) {
    cast_from_impl(*this, src);
  }
  ...
  SGL_HIP_INLINE static void memcpy(gpu_bfloat16* dst, const gpu_bfloat16* src);
};

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 1>::fill(gpu_bfloat16 val) {
  data = val;
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 1>::load(const gpu_bfloat16* ptr) {
  data = *ptr;
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 1>::store(gpu_bfloat16* ptr) const {
  *ptr = data;
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 1>::memcpy(gpu_bfloat16* dst, const gpu_bfloat16* src) {
  *dst = *src;
}
...
SGL_HIP_INLINE void vec_t<gpu_bfloat16, 2>::fill(gpu_bfloat16 val) {
  data = make_bfloat162(val, val);
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 2>::load(const gpu_bfloat16* ptr) {
  data = *((gpu_bfloat162*)ptr);
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 2>::store(gpu_bfloat16* ptr) const {
  *((gpu_bfloat162*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 2>::memcpy(gpu_bfloat16* dst, const gpu_bfloat16* src) {
...
SGL_HIP_INLINE void vec_t<gpu_bfloat16, 4>::fill(gpu_bfloat16 val) {
  *(gpu_bfloat162*)(&data.x) = make_bfloat162(val, val);
  *(gpu_bfloat162*)(&data.y) = make_bfloat162(val, val);
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 4>::load(const gpu_bfloat16* ptr) {
  data = *((uint2*)ptr);
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 4>::store(gpu_bfloat16* ptr) const {
  *((uint2*)ptr) = data;
}

SGL_HIP_INLINE void vec_t<gpu_bfloat16, 4>::memcpy(gpu_bfloat16* dst, const gpu_bfloat16* src) {
  *((uint2*)dst) = *((uint2*)src);
}
...
struct vec_t<float, 1> {
  ...
  SGL_HIP_INLINE static void memcpy(float* dst, const float* src);
};
...
SGL_HIP_INLINE void vec_t<float, 1>::memcpy(float* dst, const float* src) {
  *dst = *src;
}
...
SGL_HIP_INLINE void vec_t<float, 2>::memcpy(float* dst, const float* src) {
