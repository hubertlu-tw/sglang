...
cudaError_t BatchQKApplyRotaryPosIdsCosSinCache(
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      // constexpr uint32_t vec_size = std::max(16 / sizeof(DType), HEAD_DIM / 32); // error: constexpr variable
      // 'vec_size' must be initialized by a constant expression
