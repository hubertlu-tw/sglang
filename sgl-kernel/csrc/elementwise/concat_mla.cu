...
#ifdef USE_ROCM
constexpr int kLogicalWarpThreads = 64;
#else
...
__global__ void concat_mla_k_kernel(
  ...
  constexpr int kPayloadThreads = 32;
#ifdef USE_ROCM
  constexpr int kHardwareWave = 64;
  const int lane_id = static_cast<int>(threadIdx.x & (kPayloadThreads - 1));
  const int half_wave_id = static_cast<int>((threadIdx.x >> 5) & 1);
  const int wave_id_in_block = static_cast<int>(threadIdx.x / kHardwareWave);
  const int logical_warp_id_in_block = wave_id_in_block * 2 + half_wave_id;
  const int logical_warps_per_block = static_cast<int>((blockDim.x / kHardwareWave) * 2);
  const int flat_warp_id = blockIdx.x * logical_warps_per_block + logical_warp_id_in_block;
#else
  ...
  static_assert(sizeof(NopeVec) * kPayloadThreads == QK_NOPE_HEAD_DIM * sizeof(nv_bfloat16), "nope vec mismatch");
  static_assert(sizeof(RopeVec) * kPayloadThreads == QK_ROPE_HEAD_DIM * sizeof(nv_bfloat16), "rope vec mismatch");
...
#ifdef USE_ROCM
  constexpr int num_waves_per_block = 16;
  constexpr int logical_warps_per_wave = 2;
#else
  constexpr int num_logical_warps_per_block = 32;
#endif
#ifdef USE_ROCM
  const int grid_size = ceil_div(num_tokens * NUM_HEAD_CHUNKS, num_waves_per_block * logical_warps_per_wave);
  const int block_size = num_waves_per_block * kLogicalWarpThreads;
#else
  const int grid_size = ceil_div(num_tokens * NUM_HEAD_CHUNKS, num_logical_warps_per_block);
...
__global__ void concat_mla_absorb_q_kernel(
  ...
  static_assert(sizeof(ABufType) * A_NUM_UNROLL == A_LAST_DIM * sizeof(a[0]) / kPayloadThreads);
  ABufType a_buf[A_NUM_UNROLL];

  using BBufType = int;
  constexpr int B_NUM_UNROLL = 1;
  static_assert(sizeof(BBufType) * B_NUM_UNROLL == B_LAST_DIM * sizeof(b[0]) / kPayloadThreads);
  BBufType b_buf;
...
#pragma unroll
  for (int i = 0; i < A_NUM_UNROLL; ++i) {
    ...
    a_buf[i] = *(base_addr + i * kPayloadThreads + lane_id);
  }
...
#pragma unroll
  for (int i = 0; i < A_NUM_UNROLL; ++i) {
    ...
    *(base_addr + i * kPayloadThreads + lane_id) = a_buf[i];
  }
}
...
#endif
#ifdef USE_ROCM
  const int grid_size = ceil_div(num_items, num_waves_per_block * logical_warps_per_wave);
  const int block_size = num_waves_per_block * kLogicalWarpThreads;
#else
  const int grid_size = ceil_div(num_items, num_logical_warps_per_block);
  const int block_size = num_logical_warps_per_block * kLogicalWarpThreads;
#endif
