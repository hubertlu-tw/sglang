...
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists> rms_norm_kernel(
  ...
  auto* __restrict__ input_v =
      reinterpret_cast<const _f16Vec<scalar_t, width>*>(input + bx * static_cast<int64_t>(hidden_size));
  ...
  float variance = BlockReduce(reduceStore).Reduce(v8_variance_sum, cub::Sum{}, num_threads);
...
#define LAUNCH_FUSED_ADD_RMS_NORM(width)                                                  \
  SGL_HIP_DISPATCH_FLOATING_TYPES(input.scalar_type(), "fused_add_rms_norm_kernel", [&] { \
    sgl_hip::fused_add_rms_norm_kernel<scalar_t, width><<<grid, block, 0, stream>>>(      \
        input.data_ptr<scalar_t>(),                                                       \
